...
void	*routine(void *arg_)
{
	...
	while (!philo->someone_died)
	{
		...
		if (philo->sophers[arg->id]->is_eating && (timenow() - philo->sophers[arg->id]->time_when_started_eating) >= philo->time2eat)
		{
			log_sleeping(arg->id);
		...
		if (philo->sophers[arg->id]->is_sleeping && (timenow() - philo->sophers[arg->id]->time_when_started_sleeping) >= philo->time2sleep)
		{
